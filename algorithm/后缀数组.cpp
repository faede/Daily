...
inv get_SA()
{
    ...
    {
        sa[c[x[i]]--]=i;
        cout<<"sa :" <<"x[i]:" <<x[i] <<" "<<c[x[i]] + 1 << " : "<< i<<endl;
    }
    // 这一步sa求法 ： 去便利每个位置，根据c[]找到这个这个位置字符的排名，拿这个排名去索引sa，在索引得到的位置填上i
    ...
    for (rint k=1;k<=n;k<<=1)
    {
        ...
        //for (rint i=n;i >= n-k+1;--i) y[++num]=i;
...
inv get_height()
{
    // sa[i] 排名为 i 的子串首字母位置 
    // rk[i] 子串首字母位置为 i 的排名
    // lcp(i,j) = max_length_suff(sa[i],sa[j])
    // height[i] = lcp(i,i-1)  i是排名不是位置.   height[i]表示排名为i的和位置为i-1的 lcp 
    // h[i] = height[rk[i]]           h[i] 表示子串首字母位置为 i 的lcp
    // height[i] = h[sa[i]]
    rint k=0;
